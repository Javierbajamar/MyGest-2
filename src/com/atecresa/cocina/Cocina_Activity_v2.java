...
public class Cocina_Activity_v2 extends AppCompatActivity implements
        TextToSpeech.OnInitListener, ActionBar.OnNavigationListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.cocina_activity);
        ...
        HorizontalScrollView hv = findViewById(R.id.baseScrollView);
        //COLOR FONDO
        ...
        if (getSupportActionBar() != null) {
            ...
            getSupportActionBar().setBackgroundDrawable(colorFondoApp);
        ...
        View header = getLayoutInflater().inflate(
                R.layout.item_header_cocina_acumulado_v2, null);
        ...
        //TODO VAMOS A USAR ESTE TIMER PARA TAMBIÉN ACTIVAR EL SERVICIO SOCKET
        activarServicio();
        handler = new Handler();
    ...
    protected void onResume() {
        Inicio.setContext(Cocina_Activity_v2.this);
        super.onResume();
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_cocina_notificar:
                if (Inicio.gb.getCocina().getListaLineasSinNotificar().size() > 0) {
                    ...
                    Notifica.mostrarToastSimple(Cocina_Activity_v2.this,
    ...
    private void notificarEstadoConfig() {
        try {
            if (Inicio.gb.isImpresoraConfiguradaEnTPV()) {
                Notifica.mostrarToast(Cocina_Activity_v2.this,
    ...
    private void _mostrarComanda(ComandaCocina com, int position) {
        ...
        if (com.getLineasComanda().size() > 0) {
            ...
            //ADAPTADOR. TODO Esto pasarlo a hilo asíncrono
            ...
            //TODO Probar nuevo cambio divider
    ...
    private final AdapterView.OnItemClickListener listenerComandas = (adapterView, v, position, id) -> {
        //ada.notifyDataSetChanged(); //TODO Esto por ahora comentado. No entiendo para que lo hago aquí
    };
    ...
    private class ActualizarListaAcumulados extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            ada = new Ad_linea_acumulados_cocina(Cocina_Activity_v2.this, Inicio.gb.getCocina().getLineasAcumuladas());
            return null;
        }
    ...
    private class EjecutarFuncion extends AsyncTask<Integer, Void, Void> {
        ...
        protected void onPreExecute() {
            ...
            if (!segundoPlano) {
                try {
                    //Notifica.mostrarProgreso(Cocina_Act.this,getString(R.string.txt_dialog_cocina), getString(R.string.dialog_espere) + "...", true);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
            ...
            switch (funcion) {
                case Constantes.notificar_platos:
                    ...
                    if (Inicio.gb.getResultado().equals("Ok")) {
                        ...
                        Inicio.gb.getCocina().vaciarListaLineasSinNotificar(); //TODO PRUEBA
                    } else {
                        Notifica.mostrarMensajeEstado(Cocina_Activity_v2.this);
                    }
                    actualizarUI();
                    break;
                case Constantes.actualizar_platos:
                    ...
                    break;
                case Constantes.actualizar_acumulados:
                    ada = new Ad_linea_acumulados_cocina(Cocina_Activity_v2.this, Inicio.gb
    ...
    private void activarServicio() {
        ...
        try {
            ...
            registerReceiver(broadcastReceiverCocina, new IntentFilter(
                    ServicioSocketCnx.BROADCAST_ACTION));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private final int intervalo = 120000; // 2 MIN TODO AMPLIAMOS INTERVALO PARA ACTIVAR SOCKET
    ...
    private final Runnable runnable = new Runnable() {
        ...
        public void run() {
            try {
                //TODO, HACER ESTO AQUÍ TAL VEZ NO TOCAN LA PANTALLA
                //activarServicio(); //TODO POR AHORA LO VAMOS A DEJAR SÓLO EN EL ONCREATE
    ...
    private void salir() {
        ...
        if (Inicio.gb.getCocina().hayLineasSinNotificar()|| Inicio.gb.getCocina().hayPlatosEnPreparacion()) {
            AlertDialog.Builder alert = new AlertDialog.Builder(Cocina_Activity_v2.this);
